...
class SurepassApiClient {
  constructor() {
    ...
    this.minDelayBetweenRequests =
      parseInt(process.env.SUREPASS_MIN_DELAY_MS) || 1500; // Minimum delay configurable via env var (default 1.5 seconds)
    this.retryAttempts = 3;
    this.retryDelay = 2000; // 2 seconds base delay for retries
    this.requestsPerWindow =
      parseInt(process.env.SUREPASS_MAX_REQUESTS_PER_WINDOW) || 20; // Max requests per window (default 20)
    this.windowMs =
      parseInt(process.env.SUREPASS_RATE_LIMIT_WINDOW_MS) || 60000; // Window size in ms (default 60 seconds)
  ...
  async makeCreditCheckRequest(apiKey, endpoint, requestData) {
    return this.makeRequest(apiKey, endpoint, requestData, {
      timeout: 180000, // Credit checks might take longer
    });
  }
