...
if (!google || !google.auth || !google.sheets) {
  throw new Error(
    'Google APIs package not properly installed. Please run "npm install googleapis" in the Backend directory.',
  );
}
...
class GoogleSheetsService {
  ...
  async initialize() {
    try {
      ...
      if (!settings) {
        throw new Error(
          "Google Sheets integration not configured. Please configure the integration in the admin panel.",
        );
      }
      ...
      if (!settings.credentials) {
        throw new Error(
          "Google Sheets credentials not found. Please upload your credentials JSON file.",
        );
      }
      ...
      if (!credentials.client_email || !credentials.private_key) {
        throw new Error(
          "Invalid Google Sheets credentials. Please check your credentials JSON file.",
        );
      }
      ...
      console.log(
        "Private key length:",
        credentials.private_key ? credentials.private_key.length : 0,
      );
      ...
      console.error(
        "This usually means there is an issue with your credentials or the service account does not have access to the spreadsheet.",
      );
      console.error("Please verify that:");
      console.error("1. You have uploaded a valid credentials JSON file");
      console.error(
        "2. The Google Sheet is shared with the service account email from your credentials",
      );
      ...
      console.error(
        "Spreadsheet ID:",
        settings ? settings.spreadsheetId : "Settings not available",
      );
      console.error(
        "Solution: Make sure to share your Google Sheet with the service account email as an editor",
      );
      return false;
    }
  }
    ...
    try {
      ...
      try {
        ...
        console.error(
          ...
          getSpreadsheetError.message,
        );
        if (getSpreadsheetError.response && getSpreadsheetError.response.data) {
          console.error(
            "Spreadsheet API error details:",
            getSpreadsheetError.response.data,
          );
        }
      ...
      // console.log(
      //   "Sheets to add:",
      //   sheetsToAdd.map((req) => req.addSheet.properties.title),
      // );
      ...
      console.error(
        "This usually means the service account does not have access to the spreadsheet.",
      );
      ...
      console.error(
        "Please make sure to share your Google Sheet with the service account email:",
        serviceAccountEmail,
      );
      ...
      if (error.response && error.response.data && error.response.data.error) {
        ...
        if (apiError.code === 403) {
          ...
          console.error(
            "2. The service account email is shared with edit access to the spreadsheet",
          );
          console.error(
            "3. The credentials JSON file is valid and not expired",
          );
          console.error(
            "4. Go to your Google Sheet, click Share, and add the service account email as an EDITOR (not just viewer)",
          );
          console.error("   Service account email:", serviceAccountEmail);
          console.error(
            "   Note: Need explicit editor permissions to create/edit sheets, not just view them",
          );
        }
      }
  ...
  async syncCreditScoreData() {
    try {
      ...
      const creditReports = await CreditReport.find({ isPublic: { $ne: true } })
  ...
  async syncCreditRepairData() {
    try {
      ...
      if (
        !settings ||
        !settings.tabs ||
        !settings.tabs.get("creditScoreRepair")
      ) {
        return {
          success: false,
          message: "Credit score repair tab not configured",
        };
      }
      ...
      if (!tabSettings || !tabSettings.enabled) {
        return {
          success: false,
          message: "Credit score repair tab not enabled",
        };
      }
      ...
      const creditRepairData = await CreditRepair.find({}).sort({
        createdAt: -1,
      });
  ...
  async syncRegistrationData() {
    try {
      ...
      if (
        !settings ||
        !settings.tabs ||
        !settings.tabs.get("newRegistration")
      ) {
        return {
          success: false,
          message: "New registration tab not configured",
        };
      }
  ...
  async syncPublicCreditScoreData() {
    try {
      ...
      const creditReports = await CreditReport.find({ isPublic: true }).sort({
        createdAt: -1,
      });
      ...
      //   settings.spreadsheetId,
      //   "range:",
      //   `Website Credit Reports!A1:K${rows.length}`,
      // );
      const updateResult = await this.sheets.spreadsheets.values.update({
        spreadsheetId: settings.spreadsheetId,
  ...
  async syncFranchiseOpportunityData() {
    try {
      ...
      if (
        !settings ||
        !settings.tabs ||
        !settings.tabs.get("franchiseOpportunity")
      ) {
        return {
          success: false,
          message: "Franchise opportunity tab not configured",
        };
      }
      ...
      if (!tabSettings || !tabSettings.enabled) {
        return {
          success: false,
          message: "Franchise opportunity tab not enabled",
        };
      }
      ...
      const franchiseOpportunityData = await FranchiseOpportunity.find({}).sort(
        { createdAt: -1 },
      );
  ...
  async syncSuvidhaCentreApplicationData() {
    try {
      ...
      const suvidhaCentreApplications = await SuvidhaCentreApplication.find(
        {},
      ).sort({ createdAt: -1 });
