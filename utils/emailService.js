...
const sendAdminNotificationEmail = async (user, tempPassword, options = {}) => {
  const mailOptions = {
    from: '"Credit Dost Support" <support@creditdost.co.in> ',
    to: process.env.ADMIN_EMAIL || process.env.EMAIL_USER, // Fallback to EMAIL_USER if ADMIN_EMAIL not set
    subject:
      options.subject || "New Franchise Registration - Credit Dost Platform",
    html:
      options.html ||
      `
  ...
  try {
    ...
    if (franchise.assignedPackages && franchise.assignedPackages.length > 0) {
      ...
      const packages = await Package.find({
        _id: { $in: franchise.assignedPackages },
      });
...
const sendCreditAnalystEmail = async (user, tempPassword) => {
  try {
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: user.email,
      subject: "Credit Analyst Account Created",

      html: `
        <h2>Welcome Credit Analyst</h2>
    ...
    console.log("Credit Analyst email sent:", info.response);

    return info;
  } catch (error) {
    console.error("Error sending Credit Analyst email:", error);

    throw error;
  }
};

const sendRelationshipManagerEmail = async (user, tempPassword) => {
  try {
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: user.email,
      subject: "Relationship Manager Account Created",

      html: `
        <h2>Welcome Relationship Manager</h2>
    ...
    console.log("Relationship Manager email sent:", info.response);

    return info;
  } catch (error) {
    console.error("Error sending Relationship Manager email:", error);
...
const sendAccountCredentialsEmail = async (user, password) => {
  try {
    ...
    console.log(
      "Attempting to send account credentials email to:",
      user.email,
      "with subject:",
      mailOptions.subject,
    );
...
const sendLeadApprovalEmail = async (adminUser, lead, franchiseUser) => {
  ...
  if (adminUser.email === "nitinv@creditdost.co.in") {
    console.log(
      "Skipping lead approval email for admin user with nitinv@creditdost.co.in",
    );
    return;
  }
...
const sendLeadRejectionEmail = async (
  adminUser,
  lead,
  franchiseUser,
  reason,
  ...
  if (adminUser.email === "nitinv@creditdost.co.in") {
    console.log(
      "Skipping lead rejection email for admin user with nitinv@creditdost.co.in",
    );
    return;
  }
...
const sendBusinessFormSubmissionEmail = async (
  recipient,
  businessForm,
  franchise,
) => {
...
const sendCreditReportEmail = async (recipient, creditReport) => {
  ...
  const mailOptions = {
    ...
    html: `
      ...
      ${
        downloadLink
          ? `<p><a href="${downloadLink}" style="background-color: #6200ea; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block;">Download Report</a></p>
...
const sendJobApplicationEmail = async (applicationData, resumeBuffer) => {
  const mailOptions = {
    from: '"Credit Dost Support" <support@creditdost.co.in> ',
    to:
      process.env.JOB_APPLICATION_EMAIL ||
...
const sendAIAnalysisNotificationToAdmin = async (
  franchise,
  documentName,
  documentBuffer,
) => {
...
const sendAIAnalysisResponseToFranchise = async (
  franchise,
  documentName,
  documentBuffer,
) => {
...
const sendPackageUpgradeNotification = async (
  user,
  franchise,
  oldPackage,
  newPackage,
  ...
  try {
    ...
    console.error(
      "Error sending package upgrade notification to franchise user:",
      error,
    );
  }
  ...
  try {
    ...
    console.error(
      "Error sending package upgrade notification to admin:",
      error,
    );
  }
};
